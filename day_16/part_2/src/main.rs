...
fn process(input: &str) -> usize {
    ...
    for i in 0..map[0].len() {
        energized.push(beam_pos(&map, (i, 0), Direction::Down));
    }
    for i in 0..map[0].len() {
        energized.push(beam_pos(&map, (i, map.len() - 1), Direction::Up));
    }
    for i in 0..map.len() {
        energized.push(beam_pos(&map, (0, i), Direction::Right));
    }
    for i in 0..map.len() {
        energized.push(beam_pos(&map, (map[0].len() - 1, i), Direction::Left));
    }

    *energized.iter().max().unwrap()
}

fn beam_pos(map: &[Vec<char>], start_pos: (usize, usize), start_dir: Direction) -> usize {
